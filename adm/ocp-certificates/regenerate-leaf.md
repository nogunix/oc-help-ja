...
> OpenShift クラスタのクライアント証明書とサービング証明書を再生成する

[`oc`](../../oc.md) / [`oc adm`](../../adm.md) / [`oc adm ocp-certificates`](../ocp-certificates.md) / `regenerate-leaf`

## Usage

```
oc adm ocp-certificates regenerate-leaf [options]
```

OCP v4 クラスタが提供するリーフ証明書を再生成します。
...
<sub>`$ oc adm ocp-certificates regenerate-leaf --help` / `gen-oc-help.py` で生成</sub>

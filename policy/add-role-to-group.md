...
[`oc`](../oc.md) / [`oc policy`](../policy.md) / `add-role-to-group`

## Usage

```
oc policy add-role-to-group ROLE GROUP [GROUP ...] [flags] [options]
```

このコマンドを使うと、グループをロールに割り当てることで、現在のプロジェクト内の特定のリソースと操作へのアクセス権を付与できます。指定したロールを参照するロールバインディングを作成または変更し、対象のグループをサブジェクトの一覧に追加します。このコマンドは、対応するロールやグループのリソースが存在することを必須とせず、ロールやグループが存在しない場合や、ユーザーにそれらを参照する権限がない場合でも、バインディングの作成に成功します。
...
<sub>`$ oc policy add-role-to-group --help` / `gen-oc-help.py` で生成</sub>

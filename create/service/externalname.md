...
指定した名前で ExternalName Service を作成します。

ExternalName Service は Pod ではなく外部の DNS アドレスを参照します。これにより、アプリケーション作成者は、プラットフォーム外・別クラスタ・ローカルに存在するサービスを参照できます。

## Examples

```bash
# my-ns という名前の ExternalName Service を新規作成する

...
> 指定した名前でサービスアカウントを作成する

[`oc`](../oc.md) / [`oc create`](../create.md) / `serviceaccount`

## Usage

```
oc create serviceaccount NAME [--dry-run=server|client|none] [options]
```
...
```bash
# my-service-account という名前のサービスアカウントを新規作成する
...
<sub>`$ oc create serviceaccount --help` / `gen-oc-help.py` で生成</sub>

...
[`oc`](../oc.md) / [`oc create`](../create.md) / `quota`

## Usage

```
oc create quota NAME [--hard=key1=value1,key2=value2] [--scopes=Scope1,Scope2] [--dry-run=server|client|none] [options]
```

指定した名前・ハードリミット・スコープ（省略可）でリソースクォータを作成します。

エイリアス: quota, resourcequota

## Examples

```bash
# my-quota という名前のリソースクォータを新規作成する
oc create quota my-quota --hard=cpu=1,memory=1G,pods=2,services=3,replicationcontrollers=2,resourcequotas=1,secrets=5,persistentvolumeclaims=10

# best-effort という名前のリソースクォータを新規作成する
...
- `--hard=''`
  ハードリミットを定義する resource=quantity のペアを、カンマ区切りで指定します。
...
- `--scopes=''`
  クォータが追跡する各オブジェクトがすべて一致する必要があるクォータスコープを、カンマ区切りで指定します。
...
<sub>`$ oc create quota --help` / `gen-oc-help.py` で生成</sub>

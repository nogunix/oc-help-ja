...
[`oc`](../oc.md) / [`oc rollout`](../rollout.md) / `resume`

## Usage

```
oc rollout resume RESOURCE [options]
```

一時停止中のリソースは、コントローラーによる調整（reconcile）が行われません。リソースを再開すると、再び調整されるようになります。現在、再開に対応しているのはデプロイメントのみです。
...
<sub>`$ oc rollout resume --help` / `gen-oc-help.py` で生成</sub>
